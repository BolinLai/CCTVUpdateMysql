...
def insert_update_time(update_dicts):
    ...
    :param update_dicts: 各个表的更新日期。 格式：{table1:{source1: date1, source2: date2, ……}, table2: ……}
    """
    conn, cur = mysql_connect()
    for table in update_dicts.keys():
        for dtype in update_dicts[table].keys():
            mysql_str = "insert into {table}(table_name, source, update_date) values(%s, %s, %s) on duplicate key update update_date=values(update_date)".format(table=MysqlTable.TABLE_INFO)
...
def get_update_ymd(table, source, x=0):
    """
    获取日期。

    :param table: 数据库.数据表名
    :param source: 数据来源，如cctv5 finance
    :param x: 延迟的天数
    :return: 在数据库最后一次更新时间的基础上，提前days_ago天的日期。若数据库无更新时间，则返回None。
    """
    conn, cur = mysql_connect()
    mysql_str = "select update_date from {table} where table_name='{table_name}' and source='{source}'".format(table=MysqlTable.TABLE_INFO, table_name=table, source=source)
    cur.execute(mysql_str)
...
def get_update_ymdh(table, source, x=0):
    """
    获取时间。
...
def isdataready(date, source):
    ...
    sql = "select count(*) from {table} where ymdh={ymdh}".format(table=table, ymdh=date)
    cursor.execute(sql)
