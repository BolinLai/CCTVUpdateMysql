...
from method import TAGS, TOTAL_TAGS, NODES, get_date, get_hours, get_days, mysql_connect, mysql_close
from config import *
...
class SqlWhere:
    ...
    TOTAL_WATCHTIME_FLAG = "allwatchtime>0 and " + COMMON
    FIRST_LAST_DEVICE = "device_id is not null"
    PROGRAM = "content_id is not null and " + COMMON
...
class Data:
        ...
        if tag and tag != 'tagall':
            if tag in TAGS and TAGS[tag]:
                ...
                for subtag in TAGS[tag]:
                    tag_str += " or Lower(tagid)='{tagid}'".format(tagid=subtag)
                tag_str += ')'
            else:
                tag_str = "Lower(tagid)='{tagid}'".format(tagid=tag)
        else:
    ...
    def org_index(self):
        ...
        """
        if self.index_name in ['watchtime_dist']:
            data_dict = {}
    ...
    def trans_to_dict(self):
        ...
        """
        if self.index_name in ['play_times', 'active_devices', 'total_devices', 'watch_time', 'timevalid_active_devices', 'timevalid_total_devices']:
            if self.date_type == 'everyday' or self.date_type == 'hour':
                ...
                elif self.index_type == 'TAG' or self.index_type == 'AREA':
                    self.dicts = [{'date': item[0], 'type_id': item[2], 'area_id': item[1], 'value': int(item[3])} for item in self.data if item[2] in TOTAL_TAGS or item[2] == 'tagall']
                else:
                    pass

            elif self.date_type == '7days' or self.date_type == '30days' or self.date_type == 'month' or self.date_type == 'history':
                if self.index_type == 'PRO':
                    ...
                    self.dicts = [{'start_date': item[0], 'end_date': item[1], 'date_type': self.date_type, 'type_id': item[3], 'area_id': item[2], 'value': int(item[4])} for item in self.data if item[3] in TOTAL_TAGS or item[3] == 'tagall']
                else:
                    pass
        elif self.index_name == 'watchtime_dist':
            self.dicts = [{'start_date': item[0], 'end_date': item[1], 'date_type': self.date_type, 'type_id': item[2], 'area_id': item[3], 'value': item[4]} for item in self.data if item[2] in TOTAL_TAGS or item[2] == 'tagall']

    def revise_index(self):
        """
        对self.dict的结果进行修正，主要用于专区的一级tag的累加，对于可加的指标填上一级tag。
        """
        if (self.date_type == 'hour' or self.date_type == 'everyday') and self.dicts:
            self.dicts.sort(key=lambda a: a['date'])
            mindate = self.dicts[0]['date']
            maxdate = self.dicts[-1]['date']
            dates = get_hours(mindate, maxdate) if self.date_type == 'hour' else get_days(mindate, maxdate)

            append_data = []
            for date in dates:
                for node in NODES:
                    data_perdate = [item for item in self.dicts if item['date'] == int(date) and item['area_id'] == node]  # 注意hbase中date的格式为int而代码对date的处理都是字符串
                    for tag in TAGS.keys():
                        if TAGS[tag]:
                            for item in data_perdate:
                                if item['type_id'] == tag:
                                    item['value'] = sum([int(it['value']) for it in data_perdate if it['type_id'] in TAGS[tag] or it['type_id'] == tag])
                                    break
                            else:
                                append_data.append({'date': date, 'type_id': tag, 'area_id': node, 'value': sum([int(it['value']) for it in data_perdate if it['type_id'] in TAGS[tag] or it['type_id'] == tag])})
            self.dicts.extend(append_data)
    ...
    def cursor_execute(self, cur):
        ...
        """
        if self.index_name in ['play_times', 'active_devices', 'total_devices', 'watch_time', 'timevalid_active_devices']:
            if self.date_type == 'hour' and self.index_type == 'PRO':
                table = MysqlTable.BASIC_INDEX_HOUR_PRO
            elif self.date_type == 'hour' and (self.index_type == 'TAG' or self.index_type == 'AREA'):
            ...
            elif self.date_type in ['7days', '30days', 'month'] and (self.index_type == 'TAG' or self.index_type == 'AREA'):
            ...
            for item in self.dicts:
                if item['value'] != '[]':
                    cur.execute(mysql_str, ['watchtime_dist', item['start_date'], item['end_date'], self.date_type, item['type_id'], item['area_id'], item['value']])
            ...
            for item in self.dicts:
                if item['value'] != 0 and item['value'] != '0':
                    ...
                    if insert:
                        mysql_str = "insert into {table}(name, start_date, end_date, date_type, type_id, area_id, value) values(%s, %s, %s, %s, %s, %s, %s) on duplicate key update value=values(value)".format(table=table)
                        cur.execute(mysql_str, ['timevalid_total_devices', item['start_date'], int(item['end_date']), self.date_type, item['type_id'], item['area_id'], item['value']])
                    else:
