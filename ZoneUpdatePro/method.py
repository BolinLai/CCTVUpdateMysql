...
def get_tag():
    ...
    mysql_str = "select Lower(tag_id), Lower(parenttag) from {table};".format(table=MysqlTable.TAG)
    cur.execute(mysql_str)
    data = cur.fetchall()
...
def get_node():
    ...
    mysql_str = "select * from {table} where putaway_type=1;".format(table=MysqlTable.NODE)
    cur.execute(mysql_str)
    data = cur.fetchall()
    node = {}
    for item in data:
        if str(item[0]) == '000001':
            pass
        else:
